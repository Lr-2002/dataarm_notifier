...
class CANMetricsServer:
    ...
    - mapping: CAN ID to joint name mapping
    ...
    def _handle_metrics(self, data: Dict[str, Any]) -> None:
        ...
        if per_pair_rtt:
            ...
            for key, stats in per_pair_rtt.items():
                ...
                # Log per-pair RTT with joint names
                try:
                    send_id, recv_id = eval(key) if isinstance(key, str) else key
                    joint_name = self._can_id_to_name.get(send_id, f"joint_{send_id}")
                    rr.log(f"can/joint/{joint_name}/rtt", rr.Scalars(rtt_mean))
        ...
        for can_id_str, stats in per_id_stats.items():
            try:
                ...
                joint_name = self._can_id_to_name.get(can_id, f"id_{can_id}")
                rr.log(f"can/joint/{joint_name}/fps", rr.Scalars(fps))
                rr.log(f"can/joint/{joint_name}/jitter_p95", rr.Scalars(jitter_p95))
            except (ValueError, TypeError):
                pass
